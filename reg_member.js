...
function check(){  
    ...
    if(checkId()&&checkPw()&&checkEqualsPwAndPwRe()&&checkPw_re()&&checkUserName()&&checkEmail()){
        alert("회원가입 완료");
    } else {
        alert("회원가입 실패");
    }
...
function checkEqualsPwAndPwRe(){
    if(pw==pw_re){
        console.log("암호,암호 확인 일치")
        return true;
    }else{
        console.log("암호, 암호 확인 불일치")
        return false;
    }
}
